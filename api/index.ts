...
app.get('/capture-screenshot', async (req: any, res: { set: (arg0: string, arg1: string) => void; send: (arg0: any) => void; status: (arg0: number) => { (): any; new(): any; json: { (arg0: { error: string; }): void; new(): any; }; }; }) => {
    try {
        const fontName = req.query.font || 'Merriweather';
        ...
        const htmlContent = `
            ...
            <body>
                <h1 style="font-size: 100px;">${fontName}</h1>
            </body>
            </html>
        `;
...
async function captureScreenshot(htmlContent: string, upperPartHeight = 250) {
    ...
    try {
        await page.setContent(htmlContent); // Set HTML content directly
